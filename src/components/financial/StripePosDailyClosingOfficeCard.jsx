...
export function StripePosDailyClosingOfficeCard({
  ...
  useEffect(() => {
    if (!showFinancialTotals || !isCurrentMonthSelected(selectedMonth)) {
      setTodayTotal(null);
      return undefined;
    }
    let cancelled = false;
  ...
  }, [franchiseId, selectedMonth, showFinancialTotals]);
  ...
  const monthDisplay = showFinancialTotals
    ? (heroTotal != null ? formatCurrency(heroTotal) : (loadingMonth ? '…' : '—'))
    : '—';

  const todayDisplay = showFinancialTotals && isCurrentMonthSelected(selectedMonth)
  ...
  return (
    <div className="gm-stripe-metric-tile pal-pos-daily-office-card">
      ...
      <div className="gm-stripe-metric-tile-body">
        <span className="gm-stripe-metric-tile-value">{monthDisplay}</span>
      </div>
